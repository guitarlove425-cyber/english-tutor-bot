...
The `/academy` command starts a placement interview and then teaches the learner through six levels: Starter (A0), Elementary (A1), Pre-Intermediate (A2), Intermediate (B1), Upper-Intermediate (B2), and Advanced/Pro (C1+). The curriculum contains 60 structured lessons with grammar, vocabulary, speaking objectives, realistic tasks, checkpoint assessments, review, points, practice streaks, role-play, pronunciation coaching, and a final Pro assessment. Starter and Elementary are available as the free path; Pre-Intermediate through Advanced/Pro are Premium levels.

The bot now uses one Burmese-first navigation hierarchy instead of putting every feature on one crowded screen. The main menu contains Learning, Kids, Today’s Teacher Lesson, Practice, Progress, Profile, Beginner Course, More/Settings, and Help. Learning, Practice, Progress, Profile/Settings, Beginner Course, Privacy, Classroom, and Kids each have their own focused submenu with a visible Home button. Beginner lessons have dedicated lesson, teacher, next lesson, progress, and reset buttons. Privacy has Export and Delete buttons. Classroom has list, join-by-code, teacher create, dashboard, and Premium buttons; buttons that need a code or name ask the user for that text in the next message. Existing slash commands and older labels remain routed for backward compatibility. English target material is intentionally preserved as English while navigation, instructions, and guidance remain Burmese-first.

Learners can answer every lesson with text or voice when the phase needs it. Gemini acts as a teacher rather than only a chat partner: it explains the target language, corrects the learner, gives Burmese guidance where useful, provides pronunciation and fluency tips, asks a follow-up question, and waits for the learner to complete the current classroom step. Active lessons use progressive disclosure: the bot hides the large persistent menu and displays only the next relevant action, together with a Home button for leaving the lesson. After the learner completes that step, the bot automatically records the phase and reveals the next one. Explanations, models, understanding checks, guided practice, and review use buttons or text; voice is requested only for independent speaking or a dedicated pronunciation, live-voice, diagnostic, or role-play activity. Every active lesson also provides a pronunciation-listening button: learners can type an unfamiliar English word or sentence and receive its audio without uploading their own voice. The classroom sequence is explain → model → check understanding → guided practice → independent use → assess → homework → review; learners do not need to choose from the whole sequence at once. Teacher explanations now follow a detailed classroom standard: teach one small idea at a time; explain what it means, why it is useful, and when to use it; break English examples into parts with Burmese meaning; show two correct examples and one common mistake; check understanding before production; provide a sentence starter or guided hint; and finish with exactly one next action. The app keeps the explanation detailed but Telegram-readable instead of sending a confusing block of grammar theory. Before any long reply is sent, the bot removes raw Markdown headings, bold markers, code fences, and horizontal-rule syntax while preserving readable section spacing, bullets, English examples, and Burmese text. This prevents messages such as `###`, `**`, and `---` from appearing visibly in Telegram. Every lesson now records attempts, evidence scores, checks, best performance, weak-skill remediation notes, and a mastery state. A lesson is not marked complete merely because the learner presses a button: the learner must provide meaningful practice evidence and reach the mastery threshold, or repeat the targeted remediation. Placement is adaptive, while Premium gating prevents a free account from entering paid levels without an active Premium entitlement.
...
Privacy controls are available through the Profile/Settings button and `/privacy`. The Privacy submenu provides Export and Delete buttons. Export returns the stored profile and learning records; deletion requires an explicit confirmation button and removes the user mode, Premium record, original course progress, Academy progress, Kids progress, Word Bank, diagnostics, quizzes, and daily plans. The deletion path is designed as a user-controlled data lifecycle feature rather than an automatic destructive action.

The configured admin account also has a private Admin Center. The `🛡️ Admin Center` button is added to the Admin user's Main Menu, More/Settings menu, and Profile menu only when the Telegram user ID matches the configured `ADMIN_ID`. The Admin Center contains class listing, classroom creation, student dashboard, and manual Premium buttons. The equivalent slash commands remain available: `/classroom_create CLASS_NAME` creates a classroom and returns a six-character join code; `/classroom_join CODE` joins one; `/classroom` lists classes; `/classroom_dashboard CODE` shows student count, active learners, average completion, level, quiz accuracy, points, speaking activity, and streak; and `/upgrade USER_ID DAYS` grants Premium. The classroom dashboard reads Academy progress only and payment remains manual.

Admin protection is enforced on the server, not only in the UI. `/admin`, `/teacher`, `/classroom_create`, `/classroom_dashboard`, `/upgrade`, and all button-triggered create/dashboard/Premium prompts reject non-admin users. The text-input navigation state also checks `ADMIN_ID` before creating classrooms, reading dashboards, or granting Premium, so a user cannot bypass the hidden buttons by typing a command, guessing a route, or sending a forged follow-up message. Non-admin users see only the normal Classroom list/join controls and never receive the Admin Center keyboard.
...
The `/kids` command starts a separate child-friendly pathway with 36 lessons across six stages with six lessons per stage: Discovery English (Pre-A1, ages 5–7), Primary Foundations (Pre-A1–A1, ages 6–9), Elementary English (A1–A2, ages 8–11), Junior Communicator (A2–B1, ages 10–13), Academic Bridge (B1–B2, ages 12–15), and Young Pro English (B2–C1+, ages 14+). The age-band selector adjusts lesson language and difficulty; it does not request or store a child’s name, address, school, phone number, private photos, or other unnecessary personal information.

The Kids classroom begins with letter names, phonics, short vowels, sound blending, first-word reading, colors, and numbers. Kids explanations use tiny steps, repetition, simple Burmese, two easy models, one gentle correction, and one action at a time so a child is not overwhelmed. It then progresses through family, nature, school, manners, feelings, weekend plans, weather, stories, questions, role-play, opinions, problem stories, reading main ideas, paragraph writing, mini-presentations, research posters, projects, discussion, advanced storytelling, formal writing, presentation Q&A, a Young Pro capstone, and reflection. Each lesson stays short, uses simple Burmese instructions, preserves English target content, accepts text or voice practice, and uses the same mastery gate before allowing the next lesson.

Kids navigation is organized into clear button submenus so families do not need to type slash commands. The Kids home menu contains `📘 Kids ဒီနေ့ Lesson`, `🧰 Kids လေ့ကျင့်ခန်း`, `🔁 Kids Review`, `📊 Kids တိုးတက်မှု`, `📚 Kids အဆင့်များ`, `🧒 Kids Menu`, and `🏠 ပင်မ Menu`. The practice submenu repeats only the essential lesson, review, progress, stages, Kids Menu, and Home buttons. During an active Kids lesson, the persistent menu is hidden and only the next teacher-led action is shown, with Progress and Home always available; the next lesson button appears only after the speaking/review stage and remains subject to mastery. The Kids teacher uses encouragement rather than shame, asks for only age-band information, avoids unsafe or adult topics, and redirects personal or dangerous questions to a trusted adult. Progress reports use non-competitive language and include a simple at-home practice suggestion for a parent or trusted adult. The Kids menu also provides `👨‍👩‍👧 လူကြီးအကျဉ်းချုပ်`, a non-competitive read-only summary of lesson completion, current stage, mastery, and suggested home practice. It is not a substitute for verified guardian consent or supervision. Use `/kids`, `/kidslesson`, `/kidspractice`, `/kidsstages`, `/kidsmenu`, `/kidsprogress`, and `/kidsreview` to access the same screens if needed.

### Original Beginner Speaking Course

The `/course` command remains available as a lighter 18-lesson path from greetings and introductions through daily routines, questions, shopping, directions, and a final real-life conversation. Its teacher now explains each pattern in simple Burmese, gives word-by-word English examples and meanings, highlights one common mistake, checks understanding, and guides the learner from imitation to independent speaking. It is useful for learners who want a short path instead of the full Academy.
...
| `classrooms` | Teacher-owned classroom metadata, join codes, and student Telegram IDs for progress dashboards; write and dashboard actions are restricted to `ADMIN_ID` |
