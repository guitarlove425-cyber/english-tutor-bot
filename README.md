...
The configured admin account also has a Teacher Center. The Classroom submenu opens class listing and code-input buttons for creating a classroom, joining a classroom, viewing a dashboard, and granting manual Premium. The equivalent slash commands remain available: `/classroom_create CLASS_NAME` creates a classroom and returns a six-character join code; `/classroom_join CODE` joins one; `/classroom` lists classes; `/classroom_dashboard CODE` shows student count, active learners, average completion, level, quiz accuracy, points, speaking activity, and streak; and `/upgrade USER_ID DAYS` grants Premium. The classroom dashboard reads Academy progress only and payment remains manual.
